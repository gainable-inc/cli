---
name: gaia-code
description: Use this skill ONLY when the user EXPLICITLY opts Claude into author-mode with phrases like "code it yourself", "write the code manually", "I want YOU to do the coding", "without using Gaia / the harness / the planner", "bypass the harness", or similar explicit opt-in. This mode is EXPENSIVE — it pulls conventions, the app preflight, and a local mirror, then reads many files and runs validators. For ordinary refinement requests like "add X to view Y", "add KPIs to Z", "change the layout", "fix bug N" — use the `gaia` skill instead (cheaper; the harness's planner + BuildAgent do the work via one `gaia chat` turn).
---

# gaia code — Claude as the creative author, Gainable as the rails

## STOP — am I the right skill for this request?

Before doing anything else, check: did the user **explicitly** ask Claude to write the code itself? Look for phrases like "code it yourself", "write it manually", "I want YOU to do it", "without using the harness/Gaia/the planner", "bypass the agent".
...
1. **`gaia code pull`** — get the latest app state from the server. Two cases:
   - **Fresh workspace** (no `.gaia/` yet): pull auto-initializes — it'll prompt you to pick a project (or read it from `--project <id>` if you have one in mind), bootstrap `.gaia/project.json`, download conventions, the preflight, the writable mirror, and install this skill. One command, no setup steps.
...
7. **Framework routes are off-limits**: `routes/users.js`, `routes/agents.js`, `routes/weavy.js`, `routes/mail.js`, `routes/userFields.js`. Grep them in `./.gaia/preflight/routes/` for reference (they show patterns for auth, account scoping, etc.) but never write them.
...
9. **When the user has subjective creative ambiguity** (3 layouts that fit equally well, brand-palette decisions, "which icon"), present the choices via `AskUserQuestion`. Bounded creativity within the validators' rails — but on subjective design calls, defer to the user.
